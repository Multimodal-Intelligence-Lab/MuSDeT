...
from src.models.modules.hypercomplex.hypercomplex_layers import PHConv1d, PHMLinear
...
class GSRPHBase(nn.Module):  
    ...
    def __init__(self, n=1, in_features=1280, units=130):
        super(GSRPHBase, self).__init__()  # call the parent constructor
        self.D1 = PHMLinear(n, in_features, units)
        self.BN1 = nn.BatchNorm1d(units)
...
class EEGPHBase(nn.Module):  
    ...
    def __init__(self, n=10, in_features=10, units=1020):
        super(EEGPHBase, self).__init__()  # call the parent constructor
        self.C1 = PHConv1d(n, in_features, units, kernel_size=3, stride=1, padding=1)
        self.BN1 = nn.BatchNorm1d(units)
        self.C2 = PHConv1d(n, units, units*2, kernel_size=3, stride=1, padding=1)
        self.BN2 = nn.BatchNorm1d(units*2)
        # self.D3 = nn.Linear(units, units)
...
class ECGPHBase(nn.Module): 
    ...
    def __init__(self, n=3, in_features=3, units=513):
...
class H2(nn.Module): # aka ConvHyperNet
    ...
    def __init__(self, model_cfg, seq_dims, data_dims, n_classes, units=1024, n=4, n_eye=4, n_gsr=1, n_eeg=10, n_ecg=3):
        ...
        for k in self.modality_keys:
            if 'eeg' in k.lower():
                PHBase_dict[f'{k}'] = EEGPHBase
            elif 'ecg' in k.lower():
                PHBase_dict[f'{k}'] = ECGPHBase
            # elif 'gsr' in k.lower():
        ...
        self.PHbase = nn.ModuleDict({
            ...
            m: PHBase_dict[m](n=1, in_features=self.data_dims[m])
            for m in self.modality_keys
        })

        # n = len(self.modality_keys)
        ...
        # self.BN1 = nn.BatchNorm1d(1792)
        # self.D2 = PHMLinear(n, 3452, units)
        # self.D2 = PHMLinear(n, 6156, units) # WESAD
        concat_dim = len(self.modality_keys) * 1026  # each ECGPHBase: units=513, output=units*2=1026
        self.D2 = PHMLinear(n, concat_dim, units)
        self.BN2 = nn.BatchNorm1d(units)
        self.D3 = PHMLinear(n, units, units//2)
        self.drop2 = nn.Dropout(dropout_rate)
        self.BN3 = nn.BatchNorm1d(units//2)
    ...
    def forward(self, x_dict):
        ...
        for m in self.modality_keys:
            x = x_dict[m].transpose(1, 2)          # (B, C, D)
