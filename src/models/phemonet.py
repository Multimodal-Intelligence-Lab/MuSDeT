...
class ECGPHBase(nn.Module): 
    ...
    def __init__(self, n=3, in_features=1024, units=512):
...
class PHemoNet(nn.Module): # aka HyperNet
    ...
    def __init__(self, model_cfg, seq_dims, data_dims, n_classes, units=1024, n=4, n_eye=4, n_gsr=1, n_eeg=10, n_ecg=3):
        ...
        self.PHbase = nn.ModuleDict({
            ...
            m: PHBase_dict[m](n=1, in_features=self.data_dims[m]*self.seq_dims[m])
        ...
        concat_dim = len(self.modality_keys) * 512  # each PHBase outputs 512 units
